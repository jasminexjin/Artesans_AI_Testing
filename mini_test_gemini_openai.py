...
#OpenAI for first call and Gemini for second call
...
try:
    ...
    print(f"ERROR: Could not import Gemini or OpenAI module: {e}")
    sys.exit(1)
except Exception as e:
    print(f"ERROR: An unexpected error occurred when importing Gemini or OpenAI module: {e}")
    traceback.print_exc()
    sys.exit(1)
...
def test_openai_gemini():
    ...
    for i, row in ocr_df.iterrows():
        ...
        try:
            ...
            product_details_json = get_product_details_gemini(ocr_text)
            end_time = time.time()
            ...
            print(f"Extracted product from Gemini: {product_name}")
            print(f"Extracted expiration date from Gemini: {expiration_date}")
            ...
            print(f"First API with OpenAI took {first_time:.2f}s, Second API with Gemini took {second_time:.2f}s")
            success_count += 1
            time.sleep(30)
    ...
    # Save results
    results_file = 'mini_test_results/mini_results_gemini_openai.csv'
    ...
    summary_file = 'mini_test_results/mini_summary_gemini_openai.csv'
