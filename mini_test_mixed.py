...
def test_openai_gemini():
    ...
    for i, row in ocr_df.iterrows():
        ...
        try:
            ...
            print(f"Extracted product from OpenAI: {product_name}")
            print(f"Extracted expiration date from OpenAI: {expiration_date}")
            
            time.sleep(10)
            # Second API call - Match with existing products
            print("\nMaking second API call to match with existing products...")
            start_time = time.time()
            matching_products_json = get_matching_products_gemini(product_name, expiration_date, top_n, existing_products_df)
            end_time = time.time()
    ...
    # Save results
    results_file = 'mini_test_results/mini_results_openai_gemini.csv'
    ...
    summary_file = 'mini_test_results/mini_summary_openai_gemini.csv'
