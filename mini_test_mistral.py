...
# Try to import the Mistral module
try:
    from mistral_ai import get_product_details_mistral, get_matching_products_mistral
except ImportError as e:
    print(f"ERROR: Could not import Mistral module: {e}")
    sys.exit(1)
except Exception as e:
    print(f"ERROR: An unexpected error occurred when importing Mistral module: {e}")
    traceback.print_exc()
    sys.exit(1)
...
def test_mistral():
    ...
    for i, row in ocr_df.iterrows():
        ...
        try:
            ...
            product_details_json = get_product_details_mistral(ocr_text)
            end_time = time.time()
            ...
            start_time = time.time()
            matching_products_json = get_matching_products_mistral(product_name, expiration_date, top_n, existing_products_df)
            end_time = time.time()
    ...
    # Save results
    results_file = 'mini_test_results/mini_results_mistral.csv'
    ...
    summary_file = 'mini_test_results/mini_summary_mistral.csv'
